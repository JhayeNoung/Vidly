...
describe('/api/rentals ', ()=>{
    ...
    describe('DELETE/ ', ()=>{
        let token, id, rentaldb;
        let execPost = async()=>{
            return await request(server).delete('/api/rentals/'+id).set('x-auth-token', token);
        };
        beforeEach(async ()=>{
            token = new User({isAdmin: true}).generateAuthToken();
            rentaldb = new Rental({
        ...
        it('should return 404 if retnal is not found', async()=>{
            id = new mongoose.Types.ObjectId();
            const res = await execPost();
            expect(res.status).toBe(404);
        })

        it('should return 200 if rental has been deleted', async()=>{
            const res = await execPost();
            expect(res.status).toBe(200);
        })
